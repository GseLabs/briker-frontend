<script lang="ts">
	...
	const { class: propsClass = "", showLabel = false } = $props();
...
<Select.Root type="single" value={currentLang} onValueChange={handleLanguageChange}>
	<Select.Trigger class={cn("flex h-8 items-center gap-2", propsClass)}>
		<div class="flex items-center gap-2">
			...
			{#if showLabel}
				<span class="hidden text-sm sm:inline-block">
					{languages.find((lang) => lang.value === currentLang)?.label}
				</span>
			{/if}
		</div>
	</Select.Trigger>
