...
3. Use in components: `{m["your.message.key"]()}` or `{m['your_message_key']()}` depending on the key format.

### Translation Guidelines

- **Always add French translations to `messages/fr.json` when creating new components**
- Use consistent naming patterns (e.g., `vendors.title`, `vendors.description`)
- Add all missing translations before committing
- Use fallback values only during development, not in production code
- When adding new vendor/courier features, update the translation files immediately
...
<p>{m["login.error_required"]()}</p><p>{m["contact.form_send"]()}</p>
```
...
Write self-explanatory code instead of comments. If comments are absolutely necessary, write them in English only and keep them minimal.

## Shadcn-svelte Components

This project uses shadcn-svelte for UI components.

IMPORTANT: See the documentation at https://www.shadcn-svelte.com/llms.txt always for the latest usage guidelines.
...
Use Felte for advanced form validation and handling:
...
NEVER try to run the project with `npm run dev` or similar commands. Always ask the developer to run it for you.
ALWAYS write comments in English only. And write comments only when necessary.
